...
def constructBayesNet(gameState: hunters.GameState):
    ...
    for x in range(X_RANGE):
        for y in range(Y_RANGE):
            agentVar.append((x, y))
    obsVar = [i for i in range(0, X_RANGE + Y_RANGE + MAX_NOISE - 1)]

    variableDomainsDict[PAC] = agentVar
    variableDomainsDict[GHOST0] = agentVar
    variableDomainsDict[GHOST1] = agentVar
    variableDomainsDict[OBS0] = obsVar
    variableDomainsDict[OBS1] = obsVar
    "*** END YOUR CODE HERE ***"
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for elim in eliminationOrder:
            factorList, joinFactor = joinFactorsByVariable(factorList, elim)
            if len(joinFactor.unconditionedVariables()) > 1:
                eliminated = eliminate(joinFactor, elim)
...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        "*** YOUR CODE HERE ***"
        result = 0
        randValue = random.uniform(0, self.total())
        
        for key, value in self.items():
            if result + value > randValue:
                return key
            result += value
        "*** END YOUR CODE HERE ***"
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for pos in allPos:
            ...
            for newPos in newDist:
                distr[newPos] += self.beliefs[pos] * newDist[newPos]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        for i in range(self.numParticles):
            new = self.legalPositions[i % len(self.legalPositions)]
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for p in distr:
            prob = self.getObservationProb(observation, pacPos, p, jailPos)
            distr[p] *= prob
        if distr.total() == 0:
            self.initializeUniformly(gameState)
        else:
            ...
            distr = list(distr)
            self.particles = random.choices(distr, weights = values, k = self.numParticles)
        "*** END YOUR CODE HERE ***"
    ...
    def elapseTime(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        next = []
        allPart = self.particles
        for part in allPart:
            nextDist = self.getPositionDistribution(gameState, part)
            next.append(nextDist.sample())
        self.particles = next
        "*** END YOUR CODE HERE ***"
