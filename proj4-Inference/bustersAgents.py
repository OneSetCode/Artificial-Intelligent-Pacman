...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        maxBelief = [max(beliefs, key = lambda x: beliefs[x]) for beliefs in livingGhostPositionDistributions]
        minAction = None
        minValue = float('inf')
        for act in legal:
            value = min(self.distancer.getDistance(Actions.getSuccessor(pacmanPosition, act), belief) for belief in maxBelief)
            if value < minValue:
                minAction = act
                minValue = value
        return minAction
