...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def computeQValueFromValues(self, state, action):
        ...
        for next, prob in transFunc:
            value += prob * (self.mdp.getReward(state, action, next) + 
    ...
    def runValueIteration(self):
        ...
        for s in states:
            if not self.mdp.isTerminal(s):
                for a in self.mdp.getPossibleActions(s):
                    for t, p in self.mdp.getTransitionStatesAndProbs(s, a):
                        if p != 0 and not self.mdp.isTerminal(t):
        ...
        for i in range(self.iterations):
            ...
            for p in predecessors[state]:
                nowAction = self.computeActionFromValues(p)
                nowMaxQ = self.computeQValueFromValues(p, nowAction)
                diff = abs(self.values[p] - nowMaxQ)
